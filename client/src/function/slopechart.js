...
const slopechart_build= (svg_id, src) => {
    ...
    axios.get(src)
        .then(data__=>{
            ...
            let target_A_0 = ['A',  +document.getElementById(targets[0]).getAttribute('x1'), +document.getElementById(targets[0]).getAttribute('y1')]
            let target_B_0 = ['B',  +document.getElementById(targets[1]).getAttribute('x1'), +document.getElementById(targets[1]).getAttribute('y1')]
            let target_A_1 = ['A',  +document.getElementById(targets[0]).getAttribute('x2'), +document.getElementById(targets[0]).getAttribute('y2')]
            let target_B_1 = ['B',  +document.getElementById(targets[1]).getAttribute('x2'), +document.getElementById(targets[1]).getAttribute('y2')]



            svg.selectAll('.targets')
                .data([target_A_0, target_B_0, target_A_1, target_B_1])
                .join('line')
                .attr('x1', (d,i)=>i<=1?Number(d[1])-25:Number(d[1])+25)
                .attr('y1', d=>d[2])
                .attr('x2', (d,i)=>i<=1?Number(d[1])-13:Number(d[1])+13)
                .attr('y2', d=>d[2])
                .classed('arrow_line', true)
                .attr('stroke', d=>arrow_color[d[0]])
                .attr('stroke-width', 2)
                .attr('stroke-linecap', 'round')
                .attr('marker-end', d=>`url(#arrow_marker_${d[0]})`)
